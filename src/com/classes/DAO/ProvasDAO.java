...
//--------------------------- Verifica se a prova Existe no banco -------------------------------------       
    ...
    public boolean existe(Provas provas) {
        try {
            Connection conn = Conexao.conectar();
            String sql = "SELECT * FROM " + NOMEDATABELA + " WHERE (codprova,dia,codaluno,nota) = (?,?,?,?);";
            PreparedStatement ps = conn.prepareStatement(sql);
            ps.setInt(1, provas.getCodigoProva());
            ps.setDate(2, (Date) provas.getDia());
            ps.setInt(3, provas.getCodigoAluno());
            ps.setFloat(4, provas.getNota());
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                ps.close();
                rs.close();
